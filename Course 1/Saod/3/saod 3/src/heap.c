...
void heap_heapify(heap* h, int index)
{
    for (;;) {
        ...
        // Найти наименьший ключ : A[index], A[left] и A[right]
        int least = index;
...
int heap_decrease_key(heap* h, int index1, int newkey)
{
    index1 = h->index[index1];

    if (h->nodes[index1].key < newkey) {
        return -1;
    }
    h->nodes[index1].key = newkey;
    for (; index1 > 1 && h->nodes[index1].key < h->nodes[index1 / 2].key;
         index1 = index1 / 2) {
